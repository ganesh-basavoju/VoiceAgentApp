...
function withNativeWind(config, { input, inlineRem = 14, configPath: tailwindConfigPath = "tailwind.config", browserslist = "last 1 version", browserslistEnv = "native", typescriptEnvPath = "nativewind-env.d.ts", disableTypeScriptGeneration = false, ...options } = {}) {
    console.log("DEBUG: withNativeWind called");
        input = require('url').pathToFileURL(path_1.default.resolve(input)).href;
    debug(`input: ${input}`);
    const resolvedConfigPath = path_1.default.resolve(tailwindConfigPath);
    console.log("DEBUG: Resolving tailwind config at:", resolvedConfigPath);
    // const { important } = (0, tailwind_1.tailwindConfig)(resolvedConfigPath);
    // console.log("DEBUG: Tailwind config loaded");
